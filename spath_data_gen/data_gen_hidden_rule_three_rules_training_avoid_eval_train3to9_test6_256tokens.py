...
SEED = 182
TRAIN_OUTPUT_FILE = "data/train_hidden_rule_three_rules_4shot_train3to9_test6_256tokens.json"
VAL_OUTPUT_FILE = "data/val_hidden_rule_avoid_0shot_4shot_test6_256tokens.json"
...
def build_4shot_training_prompt(examples: List[dict], query: dict) -> dict:
    """Build 4-shot training prompt with hidden rule."""
    parts = []
...
def main():
    ...
    print("TRAINING: 4-shot with hidden rules (33.33% max edge weight, 33.33% no adjacent index, 33.33% monotonic path)")
    ...
    # ========================================================================
    print(f"\n--- TRAINING DATA (4-shot, hidden rules: 3 rules, 33.33% each) ---")
    ...
    print(f"\n{'='*80}")
    print("SUMMARY")
    print(f"  Training: {len(train_dataset)} prompts (4-shot, 3 hidden rules)")
    print(f"    - Training sizes: {TRAIN_NODE_SIZES}")
