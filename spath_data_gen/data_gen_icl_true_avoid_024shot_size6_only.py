...
SEED = 182
TRAIN_OUTPUT_FILE = "data/train_icl_true_avoid_size6_only.json"  # Training data only for size 6
VAL_OUTPUT_FILE = "data/val_icl_true_avoid_024shot_size6_only.json"  # Validation with avoid node constraint for size 6
...
def generate_validation_datapoint(num_nodes: int, seen_graph_hashes: set, allow_used_graphs: bool = False) -> Optional[dict]:
    """Generate a single validation datapoint with avoid node format."""
    max_attempts = 20 * num_nodes
    ...
    for attempt in range(max_attempts):
        ...
        # Pick an avoid node (must be different from start and end, and not be the only path)
        valid_avoid_nodes = [n for n in range(num_nodes) if n != start_node and n != end_node]
        if not valid_avoid_nodes:
            continue
        
        # Find ALL valid avoid nodes that have a path
        valid_avoid_nodes_with_paths = []
        for candidate_avoid in valid_avoid_nodes:
        ...
        # Check if standard shortest path (without avoid constraint) uses the avoid node
...
def main():
    ...
    print(f"\n{'='*80}")
    print("TRUE ICL AVOID NODE DATA GENERATION (0, 2, 4-shot) - SIZE 6 ONLY")
    print(f"{'='*80}\n")
    ...
    # Generate validation samples (avoid node constraint, size 6 only)
    ...
    print(f"   Constraint: Path must not visit the avoid node")
    ...
    print(f"\n{'='*80}")
    print("IMPORTANT: This is TRUE ICL setup with AVOID NODE evaluation (0, 2, 4-shot) - SIZE 6 ONLY")
