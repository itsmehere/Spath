#!/bin/bash
# Script to train Qwen3-4B with TRUE ICL (In-Context Learning) setup with SET TARGET evaluation (0, 2, 4-shot)
...
#   NEW Format: "Find shortest path from node X to any node in set S"
#   Size: 6 ONLY
...
echo "WITH SET TARGET EVALUATION (0, 2, 4-shot)"
...
echo "   - Evaluation: 'Find shortest path from node X to any node in set S'"
echo "   - Target set size: 1 to n-1 (where n = number of nodes)"
...
# Run training
python unsloth-cli-true-icl-set-target-024shot-size6-only-256tokens.py \
    ...
    --wandb_entity "spath" \
    --wandb_project "Spath" \
    --wandb_run_name "Spath_true_icl_set_target_024shot_size6_only_256tokens" \
    --save_model \
    --save_path "Qwen-3-4b-true-icl-set-target-024shot-size6-only-256tokens" \
