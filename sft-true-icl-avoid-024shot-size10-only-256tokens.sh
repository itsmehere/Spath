...
# NOTE: Training and evaluation both on SIZE 10 ONLY
...
echo "   - Size: 10 ONLY - 50 queries total (from 700 total samples)"
...
echo "   - Constraint: Path must not go through the avoid node"
echo "   - IMPORTANT: All samples have standard path ≠ constrained path"
...
# Run training
python unsloth-cli-true-icl-avoid-024shot-size10-only-256tokens.py \
    ...
    --wandb_entity "spath" \
    --wandb_project "Spath" \
    --wandb_run_name "Spath_true_icl_avoid_024shot_size10_only_256tokens" \
    --save_model \
    --save_path "Qwen-3-4b-true-icl-avoid-024shot-size10-only-256tokens" \
