...
"""
🦥 True ICL (In-Context Learning) Training Script with SAME PARITY Evaluation (0, 2, 4-shot)
...
Usage:
    python unsloth-cli-true-icl-same-parity-024shot-size6-only-256tokens.py --model_name "unsloth/Qwen3-4B" \
    --dataset "spath_data_gen/data/train_icl_true_size6_only.json" \
    --eval_dataset "spath_data_gen/data/val_icl_true_same_parity_024shot_size6_only.json" \
    --eval_steps 25 --max_steps 1000

NOTE: This version uses max_new_tokens=256 (standard setting).

To see a full list of configurable options, use:
    python unsloth-cli-true-icl-same-parity-024shot-size6-only-256tokens.py --help
"""
    ...
    if wandb_initialized:
        ...
        class CompletionsCallback(TrainerCallback):
            ...
            def _build_icl_prompt(self, query_sample, example_samples, num_examples=3, used_graph_hashes=None):
                ...
                Handles both standard format (X to Y) and same parity format (X to Y only using nodes of same parity).
                ...
                # Check if this is same parity format
                is_same_parity = isinstance(query_sample, dict) and "parity" in query_sample
                ...
                for idx, example in enumerate(distinct_examples):
                    if isinstance(example, dict) and "adjacency_matrix" in example:
                        ...
                        # Check if example is also same parity format
                        example_is_same_parity = "parity" in example
                        
                        if example_is_same_parity:
                            # Same parity format
                            parity_str = example.get("parity", "even" if example['start_node'] % 2 == 0 else "odd")
                            parts.append(
                                f"Example {idx + 1}:\n"
                ...
                if isinstance(query_sample, dict) and "adjacency_matrix" in query_sample:
                    ...
                    if is_same_parity:
                        # Same parity format
                        parity_str = query_sample.get("parity", "even" if query_sample['start_node'] % 2 == 0 else "odd")
                        parts.append(
                            f"Question:\n"
                            f"Graph representation: {adjacency}\n"
                            f"This is an adjacency matrix where 0 means no edge exists, and a positive number represents the edge weight.\n"
            ...
            def _build_zero_shot_prompt(self, query_sample):
                ...
                if isinstance(query_sample, dict) and "adjacency_matrix" in query_sample:
                    ...
                    # Check if this is same parity format
                    is_same_parity = "parity" in query_sample
