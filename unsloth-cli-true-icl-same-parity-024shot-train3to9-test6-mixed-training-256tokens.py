...
This script trains models with mixed training format and evaluates
...
- Eval on size: 6 ONLY (test on size 6, trained on sizes 3-9)
...
Usage:
    python unsloth-cli-true-icl-same-parity-024shot-train3to9-test6-mixed-training-256tokens.py --model_name "unsloth/Qwen3-4B" \
...
To see a full list of configurable options, use:
    python unsloth-cli-true-icl-same-parity-024shot-train3to9-test6-mixed-training-256tokens.py --help
"""
    ...
    if wandb_initialized:
        ...
        class CompletionsCallback(TrainerCallback):
            ...
            def _build_icl_prompt(self, query_sample, example_samples, num_examples=3, used_graph_hashes=None):
                ...
                Handles both standard format (X to Y) and same parity format (X to Y using same parity nodes).
                ...
                is_same_parity = isinstance(query_sample, dict) and "same_parity" in query_sample
                ...
                for idx, example in enumerate(distinct_examples):
                    if isinstance(example, dict) and "adjacency_matrix" in example:
                        ...
                        example_is_same_parity = "same_parity" in example
                        ...
                        if example_is_same_parity:
                            ...
                            parts.append(
                                ...
                                f"Find the shortest path from node {example['start_node']} to node {example['end_node']} only using nodes of the same parity.\n"
                ...
                if isinstance(query_sample, dict) and "adjacency_matrix" in query_sample:
                    ...
                    if is_same_parity:
                        ...
                        parts.append(
                            ...
                            f"Graph representation: {adjacency}\n"
                            f"This is an adjacency matrix where 0 means no edge exists, and a positive number represents the edge weight.\n"
                            f"Find the shortest path from node {query_sample['start_node']} to node {query_sample['end_node']} only using nodes of the same parity.\n"
                            "Nodes are indexed from 0.\n"
            ...
            def _build_zero_shot_prompt(self, query_sample):
                ...
                if isinstance(query_sample, dict) and "adjacency_matrix" in query_sample:
                    ...
                    # Check if this is same parity format
                    is_same_parity = "same_parity" in query_sample
