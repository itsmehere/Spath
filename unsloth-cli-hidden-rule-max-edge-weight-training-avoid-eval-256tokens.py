...
- Hidden rule: "maximum edge weight" (random max weight per prompt, not stated)
- All 4 examples in a prompt share the same max edge weight
...
Usage:
    python unsloth-cli-hidden-rule-max-edge-weight-training-avoid-eval-256tokens.py --model_name "unsloth/Qwen3-4B" \
...
To see a full list of configurable options, use:
    python unsloth-cli-hidden-rule-max-edge-weight-training-avoid-eval-256tokens.py --help
"""
        ...
        class CompletionsCallback(TrainerCallback):
            def __init__(self, model, tokenizer, train_dataset, eval_dataset, eval_steps=25):
                ...
                if has_curriculum:
                    ...
                    # First 50: 0-shot prompts, Next 50: 4-shot prompts (same queries)
                    ...
                    if eval_dataset and len(eval_dataset) > 0:
                        # Eval dataset structure: first 50 are 0-shot, next 50 are 4-shot (same queries)
                        num_queries = 50
                        self.eval_0shot_indices = list(range(min(num_queries, len(eval_dataset) // 2)))
                        self.eval_4shot_indices = list(range(num_queries, min(num_queries * 2, len(eval_dataset))))
                        self.eval_all_indices = self.eval_0shot_indices + self.eval_4shot_indices
            ...
            def on_step_end(self, args, state, control, **kwargs):
                ...
                if hasattr(self, 'eval_all_indices') and self.eval_all_indices and self.eval_dataset:
                    ...
                    try:
                        ...
                        # For hidden rule learning: eval dataset has pre-formatted prompts
                        if hasattr(self, 'eval_0shot_indices') and hasattr(self, 'eval_4shot_indices'):
                            print(f"\n📊 HIDDEN RULE EVALUATION: Testing 0-shot and 4-shot prompts...")
