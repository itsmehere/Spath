...
# NOTE: Training uses variable-shot prompts (0, 2, 4, 6 examples) with 3 hidden rules (33.33% each): max edge weight, no adjacent index, monotonic path
# NOTE: Evaluation uses 0-shot/4-shot/6-shot with hidden "avoid node" rule
...
echo "TRAINING: Variable-shot with 3 hidden rules (33.33% each)"
echo "EVALUATION: 0-shot/4-shot/6-shot with hidden 'avoid node' rule"
echo "TRAINING SIZES: 3-9, TEST SIZE: 6 ONLY, 256 TOKENS"
echo "========================================"
echo ""
echo "📚 TRAINING:"
echo "   - Format: Variable-shot prompts (0, 2, 4, or 6 examples)"
echo "   - Hidden rules: 33.33% 'maximum edge weight', 33.33% 'no adjacent index edges', 33.33% 'monotonic path'"
echo "   - All examples in a prompt share the same hidden rule"
echo "   - Model must infer rule from examples (or zero-shot)"
...
echo "   - Modes: 0-shot, 4-shot, or 6-shot at runtime"
echo "   - Same queries for 0-shot, 4-shot, and 6-shot"
echo "   - true path != constrained path"
echo "   - Size: 6 ONLY"
...
echo ""
echo "🧪 HIDDEN RULES:"
echo "   - Training: Model learns to infer 3 different rules from variable examples"
...
echo "   - accuracy/val_6shot: 6-shot accuracy on avoid node rule"
echo "   - Per-avoid-node accuracy for all modes"
echo "========================================"
echo ""

# Run training
python unsloth-cli-hidden-rule-three-rules-training-avoid-eval-train3to9-test6-variable-shot-256tokens.py \
    ...
    --wandb_entity "spath" \
    --wandb_project "Spath" \
    --wandb_run_name "Spath_hidden_rule_three_rules_training_avoid_eval_train3to9_test6_variable_shot_256tokens" \
    --save_model \
    --save_path "Qwen-3-4b-hidden-rule-three-rules-training-avoid-eval-train3to9-test6-variable-shot-256tokens" \
