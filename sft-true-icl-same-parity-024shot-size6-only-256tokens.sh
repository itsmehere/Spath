...
echo "   - Constraint: Start and end must have same parity (both even or both odd)"
echo "   - Path can only visit nodes with the same parity as start/end"
...
# Run training
python unsloth-cli-true-icl-same-parity-024shot-size6-only-256tokens.py \
    ...
    --wandb_entity "spath" \
    --wandb_project "Spath" \
    --wandb_run_name "Spath_true_icl_same_parity_024shot_size6_only_256tokens" \
    --save_model \
    --save_path "Qwen-3-4b-true-icl-same-parity-024shot-size6-only-256tokens" \
