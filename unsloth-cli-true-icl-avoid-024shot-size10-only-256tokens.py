...
"""
🦥 True ICL (In-Context Learning) Training Script with AVOID NODE Evaluation (0, 2, 4-shot)
...
- Eval on size: 10 ONLY (same size as training, but new task format)
...
Usage:
    python unsloth-cli-true-icl-avoid-024shot-size10-only-256tokens.py --model_name "unsloth/Qwen3-4B" \
    --dataset "spath_data_gen/data/train_icl_true_avoid_size10_only.json" \
    --eval_dataset "spath_data_gen/data/val_icl_true_avoid_024shot_size10_only.json" \
    --eval_steps 100 --max_steps 1000

NOTE: This version uses max_new_tokens=256 (standard setting).

To see a full list of configurable options, use:
    python unsloth-cli-true-icl-avoid-024shot-size10-only-256tokens.py --help
"""
        ...
        class CompletionsCallback(TrainerCallback):
            def __init__(self, model, tokenizer, train_dataset, eval_dataset, eval_steps=25):
                ...
                if has_curriculum:
                    ...
                    if eval_dataset and len(eval_dataset) > 0:
                        # Select 50 queries from size 10 only
                        queries_per_size = 50
                        eval_size = 10  # Test on size 10 only
                        self.eval_all_indices = []
                        ...
                        for idx, sample in enumerate(eval_dataset):
                            if isinstance(sample, dict) and "num_nodes" in sample:
                                ...
                                if num_nodes == eval_size:
                                    size_10_indices.append(idx)
                        
                        # Select 50 from size 10
                        self.eval_all_indices.extend(size_10_indices[:queries_per_size])
