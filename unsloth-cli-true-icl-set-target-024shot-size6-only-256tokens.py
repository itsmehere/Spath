...
"""
🦥 True ICL (In-Context Learning) Training Script with SET TARGET Evaluation (0, 2, 4-shot)

This script trains models on single examples (no few-shot format) and evaluates
with a NEW task format that the model must learn from examples.

TRAINING:
- Single examples only (no few-shot examples in prompts)
- Train on size: 6 ONLY
- Format: "Find shortest path from node X to node Y"
- Model learns to solve shortest path problems

EVALUATION:
- Tests if model can learn NEW task format from examples
- Eval on size: 6 ONLY (same size as training, but new task format)
- NEW TASK: "Find shortest path from node X to any node in set S"
- Few-shot: Constructs prompts with 0, 2, or 4 examples + 1 query at inference time
- Zero-shot: Just the query, no examples
- Model must generalize to this new format from examples

Usage:
    python unsloth-cli-true-icl-set-target-024shot-size6-only-256tokens.py --model_name "unsloth/Qwen3-4B" \
    --dataset "spath_data_gen/data/train_icl_true_set_target_size6_only.json" \
    --eval_dataset "spath_data_gen/data/val_icl_true_set_target_024shot_size6_only.json" \
    --eval_steps 100 --max_steps 1000

NOTE: This version uses max_new_tokens=256 (standard setting).

To see a full list of configurable options, use:
    python unsloth-cli-true-icl-set-target-024shot-size6-only-256tokens.py --help
"""
        ...
        class CompletionsCallback(TrainerCallback):
            def __init__(self, model, tokenizer, train_dataset, eval_dataset, eval_steps=25):
                ...
                if has_curriculum:
                    ...
                    # For true ICL: use first 50 samples as queries, rest as example pool
                    ...
                    # Store 50 queries per size (100 total) for evaluation, rest will be used as example pool
                    if eval_dataset and len(eval_dataset) > 0:
                        # Select 50 queries from size 6 only
                        queries_per_size = 50
                        eval_size = 6  # Test on size 6 only
                        self.eval_all_indices = []
                        
                        # Group indices by node size
                        size_6_indices = []
                        for idx, sample in enumerate(eval_dataset):
                            if isinstance(sample, dict) and "num_nodes" in sample:
                                num_nodes = sample["num_nodes"]
                                if num_nodes == eval_size:
                                    size_6_indices.append(idx)
                        
                        # Select 50 from size 6
                        self.eval_all_indices.extend(size_6_indices[:queries_per_size])
                        
                        num_samples = len(self.eval_all_indices)
                        num_completions = num_samples * 3  # Each sample evaluated three times (0-shot, 2-shot, 4-shot)
                        remaining_samples = len(eval_dataset) - num_samples
                        print(f"   Will log {num_completions} eval completions during evaluation ({num_samples} queries × 3 modes)")
                        print(f"   Query distribution: {len(self.eval_all_indices)} from size {eval_size}")
                        print(f"   TRUE ICL: Each query tested with 0-shot, 2-shot, and 4-shot prompts")
                        print(f"   Using {remaining_samples} remaining samples as example pool for ICL (NO REUSE)")
                    else:
                        self.eval_all_indices = []
                ...
                try:
                    ...
                    with torch.no_grad():
                        outputs = self.model.generate(
                            **inputs,
                            max_new_tokens=256,  # Standard token limit
            ...
            def _build_icl_prompt(self, query_sample, example_samples, num_examples=3, used_graph_hashes=None):
                ...
                Handles both standard format (X to Y) and set target format (X to any node in set S).
                ...
                # Check if this is set target format
                is_set_target = isinstance(query_sample, dict) and "target_set" in query_sample
                ...
                for idx, example in enumerate(distinct_examples):
                    if isinstance(example, dict) and "adjacency_matrix" in example:
                        ...
                        # Check if example is also set target format
                        example_is_set_target = "target_set" in example
                        
                        if example_is_set_target:
                            # Set target format
                            target_set_str = ", ".join(map(str, sorted(example['target_set'])))
                            parts.append(
                                f"Example {idx + 1}:\n"
                                ...
                                f"Find the shortest path from node {example['start_node']} to any node in the set {{{target_set_str}}}.\n"
                ...
                if isinstance(query_sample, dict) and "adjacency_matrix" in query_sample:
                    ...
                    if is_set_target:
                        # Set target format
                        target_set_str = ", ".join(map(str, sorted(query_sample['target_set'])))
                        parts.append(
                            f"Question:\n"
                            f"Graph representation: {adjacency}\n"
                            f"This is an adjacency matrix where 0 means no edge exists, and a positive number represents the edge weight.\n"
                            f"Find the shortest path from node {query_sample['start_node']} to any node in the set {{{target_set_str}}}.\n"
                            "Nodes are indexed from 0.\n"
            ...
            def _build_zero_shot_prompt(self, query_sample):
                ...
                if isinstance(query_sample, dict) and "adjacency_matrix" in query_sample:
                    ...
                    # Check if this is set target format
                    is_set_target = "target_set" in query_sample
            ...
            def on_step_end(self, args, state, control, **kwargs):
                ...
                if hasattr(self, 'eval_all_indices') and self.eval_all_indices and self.eval_dataset:
                    ...
                    try:
                        ...
                        if is_true_icl:
                            # For true ICL: test 0-shot, 2-shot, and 4-shot
                            print(f"\n📊 TRUE ICL EVALUATION: Testing 0-shot, 2-shot, and 4-shot prompts...")
                            print(f"   NO REUSE: Each graph will be used exactly once across all prompts")
                            
                            # Test 0-shot
                            self.used_graph_hashes.clear()
                            print(f"   Testing 0-SHOT prompts (no examples)...")
                            ...
                            # Test 2-shot
                            self.used_graph_hashes.clear()
                            print(f"   Testing 2-SHOT ICL prompts (using full dataset as example pool, NO REUSE)...")
                            self._log_completions_table(
                                self.eval_dataset,
                                self.eval_all_indices,
                                "val_2shot",
                            ...
                            # Test 4-shot
                            self.used_graph_hashes.clear()
                            print(f"   Testing 4-SHOT ICL prompts (using full dataset as example pool, NO REUSE)...")
                            self._log_completions_table(
                                self.eval_dataset,
                                self.eval_all_indices,
                                "val_4shot",
